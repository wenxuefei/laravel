...
@section("content")
    ...
    <div class="col-sm-8">
        <blockquote>
            <p>{{$topic->name}}</p>
            <footer>文章：{{$topic->article()->count()}}</footer>
                ...
                <div class="tab-pane active" id="tab_1">
                    @foreach($posts as $post)
                        <div class="blog-post" style="margin-top: 30px">
                            ...
                            <p class=""><a href="/user/{{$post->user_id}}">{{$post->user->username}}</a> {{$post->created_at->diffForHumans()}}</p>
                            <p class=""><a href="/article/{{$post->id}}" >{{$post->title}}</a></p>
